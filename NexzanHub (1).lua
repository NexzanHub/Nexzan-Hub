...
local DefaultOptions = {
	Discord = true, -- ikon Discord (statistik server + tombol Join)
...
--// ==========================================================================
--//  16. DISCORD PANEL — statistik server real-time + tombol Join
--// ==========================================================================

local DiscordPanel = {}

--// --------------------------------------------------------------------------
--//  KONFIGURASI — ubah di sini saja bila ganti server / banner
--// --------------------------------------------------------------------------

local DISCORD = {
	Invite = "https://discord.gg/DNhYgwR9SZ",
	Code = "DNhYgwR9SZ",
	Banner = "https://files.catbox.moe/i2owju.png",

	-- Interval penyegaran statistik (detik). Jangan terlalu kecil supaya
	-- tidak kena rate limit Discord.
	RefreshInterval = 60,

	-- Tinggi area banner.
	BannerHeight = 160,

	-- "Fit"     = seluruh gambar terlihat (aman untuk logo, tidak terpotong)
	-- "Crop"    = memenuhi area, sisi berlebih dipotong (cocok untuk banner lebar)
	-- "Stretch" = dipaksa memenuhi (gambar bisa gepeng)
	BannerScale = "Fit",
}

DiscordPanel.Config = DISCORD

--// --------------------------------------------------------------------------
--//  Pengambilan data (dengan cache + anti spam request)
--// --------------------------------------------------------------------------
...
DiscordPanel.Stats = GuildStats

local API_URL = "https://discord.com/api/v10/invites/%s?with_counts=true"

--- Ambil statistik server. Selalu async, tidak pernah memblokir UI.
--- onDone(success, stats) dipanggil setelah selesai.
local function fetchStats(onDone)
	if GuildStats.Fetching then
		return
	end
	GuildStats.Fetching = true

	task.spawn(function()
		local url = string.format(API_URL, DISCORD.Code)
		...
		-- fallback ke request() bila HttpGet diblokir executor
		...
		GuildStats.Fetching = false
		GuildStats.LastFetch = os.clock()

		if not body then
			GuildStats.Error = "Tidak dapat menghubungi Discord"
			if onDone then
				onDone(false, GuildStats)
			end
			return
		end
		...
		if not okDecode or type(data) ~= "table" then
			GuildStats.Error = "Respons Discord tidak valid"
			if onDone then
				onDone(false, GuildStats)
			end
			return
		...
		-- invite dihapus / kedaluwarsa
		if data.code == 10006 or data.message then
			GuildStats.Error = "Invite tidak valid atau kedaluwarsa"
			if onDone then
				onDone(false, GuildStats)
			end
			return
		...
		GuildStats.Name = guild.name or GuildStats.Name
		GuildStats.Members = tonumber(data.approximate_member_count) or 0
		GuildStats.Online = tonumber(data.approximate_presence_count) or 0
		GuildStats.Description = guild.description
		GuildStats.Error = nil
		GuildStats.Fetched = true

		if guild.id and guild.icon then
			GuildStats.IconUrl = string.format(
		...
		if onDone then
			onDone(true, GuildStats)
		end
	end)
end

DiscordPanel.Fetch = fetchStats

--// --------------------------------------------------------------------------
--//  Buka / salin invite
--// --------------------------------------------------------------------------

--- Coba buka Discord langsung. Kalau executor tidak mendukung,
--- link disalin ke clipboard sebagai cadangan.
function DiscordPanel.Join()
	local invite = DISCORD.Invite

	local ok = false

	-- request ke Discord RPC lokal (cara paling mulus, tanpa keluar game)
	local req = (syn and syn.request) or (http and http.request) or http_request or request
	if req then
		local okRpc = pcall(function()
			req({
	...
	-- 2) Selalu salin ke clipboard juga, supaya pengguna tetap punya link
	local copied = Util.Clipboard(invite)
...
function DiscordPanel.Init(ctx)
	local C = Kit.Controls
	local live = {}
	local bannerImage
	local refreshTaskName = "nexzan_discord_stats"
	...
	local panel = Panel.new(ctx, {
		...
		Build = function(self)
			...
			-- placeholder saat banner diunduh
			local placeholder = Kit.Card(12, {
				Size = UDim2.new(1, 0, 1, 0),
				ThemeTag = { ImageColor3 = "Text" },
				ImageTransparency = 0.94,
				ZIndex = 1210,
			...
			Kit.Outline(12, { ZIndex = 1212, ImageTransparency = 0.9 }).Parent = bannerHolder

			--// Banner diunduh async supaya panel langsung tampil.
			task.spawn(function()
				local asset = DiscordPanel.ResolveImage(DISCORD.Banner)
				if asset and bannerImage and bannerImage.Parent then
					bannerImage.Image = asset
					Kit.Anim(bannerImage, 0.3, { ImageTransparency = 0 })
					Kit.Anim(placeholder, 0.3, { ImageTransparency = 1 })
				end
			end)

			--// ---- Statistik live ----
			C.Section(body, "Server", 2)

			live.name = C.Info(body, {
				Title = "Nama",
				Value = GuildStats.Name,
				LayoutOrder = 3,
			})
			live.members = C.Info(body, {
				Title = "Total Member",
				Value = GuildStats.Fetched and Util.Comma(GuildStats.Members) or "Memuat...",
				LayoutOrder = 4,
			})
			live.online = C.Info(body, {
				Title = "Sedang Online",
				Value = GuildStats.Fetched and Util.Comma(GuildStats.Online) or "Memuat...",
				LayoutOrder = 5,
			})
			live.status = C.Info(body, {
				Title = "Status",
				Value = "Menghubungkan...",
				LayoutOrder = 6,
			})
			...
			--// ---- Aksi ----
			C.Section(body, "Aksi", 10)

			C.Button(body, {
				Title = "Join Discord",
				Icon = "external-link",
				LayoutOrder = 11,
				Tooltip = DISCORD.Invite,
				Callback = function()
					local opened, copied = DiscordPanel.Join()
					if opened then
						Nexzan.Notify("Discord", "Membuka Discord...", "message-circle")
					elseif copied then
						Nexzan.Notify("Discord", "Link disalin — tempel di browser", "clipboard-check")
					else
						Nexzan.Notify("Discord", DISCORD.Invite, "message-circle", 8)
					end
				end,
			})
			...
			C.Button(body, {
				...
				LayoutOrder = 12,
				Callback = function()
					if Util.Clipboard(DISCORD.Invite) then
						Nexzan.Notify("Discord", "Link disalin ke clipboard", "clipboard-check")
					else
						Nexzan.Notify("Discord", DISCORD.Invite, "message-circle", 8)
					end
				end,
			...
			C.Button(body, {
				Title = "Segarkan Statistik",
				Icon = "refresh-cw",
				LayoutOrder = 13,
				Callback = function()
					DiscordPanel.Refresh(true)
				end,
			})

			--// muat data pertama kali
			DiscordPanel.Refresh(false)
		end,

		OnOpen = function()
			-- Segarkan saat dibuka bila data sudah basi.
			if os.clock() - GuildStats.LastFetch > DISCORD.RefreshInterval then
				DiscordPanel.Refresh(false)
			end

			-- Penyegaran berkala HANYA saat panel terbuka (hemat jaringan).
			Util.Scheduler.Add(refreshTaskName, DISCORD.RefreshInterval, function()
				DiscordPanel.Refresh(false)
			end)
		end,

		OnClose = function()
			Util.Scheduler.Remove(refreshTaskName)
		end,
	})

	--// Perbarui label dari data terbaru.
	function DiscordPanel.Refresh(notify)
		if live.status then
			live.status:Set(GuildStats.Fetching and "Memuat..." or "Memuat...")
		end

		fetchStats(function(success)
			if live.name then
				live.name:Set(GuildStats.Name)
			end

			if success then
				if live.members then
					live.members:Set(Util.Comma(GuildStats.Members))
				end
				if live.online then
					live.online:Set(Util.Comma(GuildStats.Online) .. " online")
				end
				if live.status then
					live.status:Set("Terhubung · " .. Util.TimeString())
				end
				if notify then
					Nexzan.Notify(
						"Discord",
						("%s member · %s online"):format(Util.Comma(GuildStats.Members), Util.Comma(GuildStats.Online)),
						"refresh-cw"
					)
				end
			else
				...
				if live.status then
					live.status:Set(GuildStats.Error or "Gagal memuat")
				end
				if notify then
					Nexzan.Notify("Discord", GuildStats.Error or "Gagal memuat statistik", "triangle-alert")
				end
			end

			if panel and panel.Opened then
				panel:Refresh()
			end
		end)
	end

	ctx.Panels.Discord = panel
	return panel
end

--// --------------------------------------------------------------------------
--//  Resolve gambar URL -> asset Roblox (download + getcustomasset)
--// --------------------------------------------------------------------------

local imageCache = {}

function DiscordPanel.ResolveImage(source)
	...
	if not Util.File.Enabled or not getcustomasset then
		return nil
	end
	...
	if #safe > 60 then
		safe = safe:sub(1, 30) .. "_" .. tostring(#source) .. safe:sub(-16)
	end

	local folder = NEXZAN_FOLDER .. "/assets"
	Util.File.Ensure(NEXZAN_FOLDER)
	...
	if not isfile(path) then
		...
		if not Util.Try(writefile, path, body) then
			return nil
		end
	end
...
local function attachToWindow(ctx)
	...
	--// 0) Discord
	if ctx.Options.Discord then
		local panel = DiscordPanel.Init(ctx)
		Header.Add("Discord", "Discord Server", function()
			if ctx.Search then
				ctx.Search:Close()
			end
			panel:Toggle()
